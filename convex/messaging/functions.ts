...
const deliveryView = v.object({
  ...
  recipient: v.string(), body: v.string(), attemptCount: v.number(), createdAt: v.number(), providerRequestId: v.union(v.string(), v.null()),
});
...
export const getForDelivery = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    return message ? { messageId: message._id, status: message.status, recipient: message.normalizedRecipient, body: message.body, attemptCount: message.attemptCount, createdAt: message.createdAt, providerRequestId: message.providerRequestId ?? null } : null;
  },
});
...
export const list = query({
  ...
  returns: v.object({ page: v.array(v.object({ messageId: v.id("smsMessages"), eventType: v.string(), recipient: v.string(), status: v.string(), segmentEstimate: v.number(), attemptCount: v.number(), providerStatus: v.union(v.string(), v.null()), createdAt: v.number() })), ...paginationResultFields }),
  handler: async (ctx, args) => {
    ...
    return { ...page, page: page.page.map((row) => ({ messageId: row._id, eventType: row.eventType, recipient: row.normalizedRecipient, status: row.status, segmentEstimate: row.segmentEstimate, attemptCount: row.attemptCount, providerStatus: row.providerStatus ?? null, createdAt: row.createdAt })) };
  },
});
