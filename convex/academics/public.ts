...
export const getCourseBySlug = query({ args: { slug: v.string() }, returns: v.union(publicCourseDetail, v.null()), handler: async (ctx, args) => { const course = await ctx.db.query("courses").withIndex("by_slug", q => q.eq("slug", normalizeSlug(args.slug))).unique(); if (!course || !course.isPublic || course.status !== "active") return null; const links = await ctx.db.query("courseSubjects").withIndex("by_courseId_and_sortOrder", q => q.eq("courseId", course._id)).take(100); const subjects = []; for (const link of links) { const subject = await ctx.db.get("subjects", link.subjectId); if (subject?.status === "active") subjects.push({ id: subject._id, code: subject.code, nameBn: subject.nameBn, nameEn: subject.nameEn, sortOrder: link.sortOrder }); } return { ...projectCourse(course as never), subjects }; } });
