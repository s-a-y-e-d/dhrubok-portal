...
test("monthly due day must stay within 1 to 28", async () => {
  ...
  await expect(owner.mutation(api.settings.updateOperations, { expectedUpdatedAt: storedUpdatedAt, monthlyDueDay: 29, defaultLocale: "bn", defaultGuardianSmsLocale: "bn", publicAdmissionsOpen: false, smsEnabled: false, receiptFooterBn: "ধন্যবাদ", receiptFooterEn: "Thanks" })).rejects.toThrow("between 1 and 28");
});
...
test("optimistic concurrency rejects out-of-date updates", async () => {
  ...
  await expect(owner.mutation(api.settings.updateOperations, { expectedUpdatedAt: storedUpdatedAt - 1, monthlyDueDay: 15, defaultLocale: "bn", defaultGuardianSmsLocale: "bn", publicAdmissionsOpen: false, smsEnabled: false, receiptFooterBn: "ধন্যবাদ", receiptFooterEn: "Thanks" })).rejects.toThrow("modified by another owner");

  // Succeeds when expectedUpdatedAt is current
  await owner.mutation(api.settings.updateOperations, { expectedUpdatedAt: storedUpdatedAt, monthlyDueDay: 20, defaultLocale: "bn", defaultGuardianSmsLocale: "bn", publicAdmissionsOpen: false, smsEnabled: false, receiptFooterBn: "ধন্যবাদ", receiptFooterEn: "Thanks" });

  const updatedStored = await t.run(async (ctx) => (await ctx.db.query("coachingSettings").take(1))[0]);
  expect(updatedStored.monthlyDueDay).toBe(20);
});
