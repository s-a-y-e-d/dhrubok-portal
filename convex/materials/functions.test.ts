...
async function seed(t: ReturnType<typeof convexTest>) {
  return await t.run(async (ctx) => {
    ...
    for (const [index, batchId] of [batchA, batchB].entries()) {
      ...
      await ctx.db.insert("enrolments", { studentId, courseId, batchId, academicSessionId, enrolledOn: "2026-01-01", status: "active", createdAt: now, updatedAt: now, createdByAccountId: ownerId });
