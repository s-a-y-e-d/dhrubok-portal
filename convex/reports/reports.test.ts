...
async function seed(t: ReturnType<typeof convexTest>): Promise<Fixture> {
  return await t.run(async (ctx) => {
    ...
    const batchOneId = await ctx.db.insert("batches", { academicSessionId, courseId, code: "B1", slug: "b1", nameBn: "ব্যাচ এক", nameEn: "Batch One", status: "active", admissionOpen: true, isPublic: true, publicSortOrder: 1, createdAt: now, updatedAt: now });
    const batchTwoId = await ctx.db.insert("batches", { academicSessionId, courseId, code: "B2", slug: "b2", nameBn: "ব্যাচ দুই", nameEn: "Batch Two", status: "active", admissionOpen: true, isPublic: true, publicSortOrder: 2, createdAt: now, updatedAt: now });
