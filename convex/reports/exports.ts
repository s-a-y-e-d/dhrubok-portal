...
import {
  ...
  localized,
  REPORT_ROW_LIMIT,
  requireOwnerOrAssignedTeacherForBatch,
  requireOwnerOrStudent,
} from "./shared";

const exportPayload = v.object({
  filename: v.string(),
  contentType: v.literal("text/csv;charset=utf-8"),
  content: v.string(),
  rowCount: v.number(),
  truncated: v.boolean(),
});
...
export const collectionsCsv = query({
  ...
  handler: async (ctx, args) => {
    ...
    return {
      filename: `collections-${args.fromAt}-${args.toAt}-${args.locale}.csv`,
      contentType: "text/csv;charset=utf-8" as const,
      content: createCsv([header, ...body]),
  ...
  handler: async (ctx, args) => {
    ...
    for (const summary of rows) {
      ...
      body.push([
        ...
        summary.advanceCreditMinor,
      ]);
    }
    return {
      filename: `dues-${args.locale}.csv`,
      contentType: "text/csv;charset=utf-8" as const,
      content: createCsv([header, ...body]),
      rowCount: rows.length,
      truncated,
...
export const attendanceCsv = query({
  ...
  handler: async (ctx, args) => {
    ...
    const source = await ctx.db
      ...
      .withIndex("by_batchId_and_submittedAt", (q) =>
        q
          .eq("batchId", args.batchId)
          .gte("submittedAt", args.fromAt)
          .lt("submittedAt", args.toAt),
      )
      .take(REPORT_ROW_LIMIT + 1);
    const statusRows = args.status
      ? source.filter((record) => record.status === args.status)
      : source;
    const truncated = source.length > REPORT_ROW_LIMIT;
    const rows = statusRows.slice(0, REPORT_ROW_LIMIT);
    const header =
      args.locale === "bn"
        ? ["তারিখ", "শিক্ষার্থী নম্বর", "শিক্ষার্থীর নাম", "অবস্থা"]
        : ["Date", "Student number", "Student name", "Status"];
    const body = [];
    for (const record of rows) {
      const [student, session] = await Promise.all([
        ctx.db.get("students", record.studentId),
        ctx.db.get("classSessions", record.sessionId),
      ]);
      body.push([
        session?.sessionDate ?? "",
        student?.studentNumber ?? "",
    ...
    return {
      filename: `attendance-${args.batchId}-${args.locale}.csv`,
      contentType: "text/csv;charset=utf-8" as const,
      content: createCsv([header, ...body]),
      rowCount: rows.length,
      truncated,
...
export const examResultsCsv = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (account.role === "owner") await requireOwner(ctx);
      ...
      assignedBatchId =
        (await requireTeacherExamAssignment(ctx, args.examId)).assignment
          .batchId ?? null;
    else throw new Error("Unauthorized");
    const source = await ctx.db
      .query("examResults")
      .withIndex("by_examId_and_entryStatus_and_totalScoreScaled", (q) =>
    ...
    const scoped =
      assignedBatchId == null
        ...
        : (
            await Promise.all(
              source.map(async (result) => ({
                result,
                enrolment: await ctx.db.get("enrolments", result.enrolmentId),
              })),
            )
          )
            .filter(({ enrolment }) => enrolment?.batchId === assignedBatchId)
            .map(({ result }) => result);
    ...
    return {
      filename: `exam-results-${args.examId}-${args.locale}.csv`,
      contentType: "text/csv;charset=utf-8" as const,
      content: createCsv([header, ...body]),
      rowCount: rows.length,
      truncated,
...
export const studentStatementCsv = query({
  ...
  handler: async (ctx, args) => {
    ...
    const body = rows.map((charge) => [
      charge.chargeNumber,
      localized(args.locale, charge.descriptionBn, charge.descriptionEn),
      charge.netAmountMinor,
      charge.paidAmountMinor,
      charge.dueDate,
      charge.status,
    ]);
    return {
      filename: `statement-${student.studentNumber}-${args.locale}.csv`,
      contentType: "text/csv;charset=utf-8" as const,
      content: createCsv([header, ...body]),
      rowCount: rows.length,
      truncated,
...
export const receivableAgeingCsv = query({
  args: { locale: localeArg },
  returns: exportPayload,
  handler: async (ctx, args) => {
    await requireOwner(ctx);
    const source = await ctx.db
      ...
      .withIndex("by_courseId_and_overdueMinor")
      .order("desc")
      .take(REPORT_ROW_LIMIT + 1);
    const { rows, truncated } = boundedResult(source);
    const header = [
    ...
    for (const row of rows) {
      const [student, course, batch] = await Promise.all([
        ...
        row.courseId ? ctx.db.get("courses", row.courseId) : null,
        row.batchId ? ctx.db.get("batches", row.batchId) : null,
      ]);
      body.push([
        student?.studentNumber ?? "",
        student?.displayName ?? "",
        course ? localized(args.locale, course.nameBn, course.nameEn) : "",
        batch ? localized(args.locale, batch.nameBn, batch.nameEn) : "",
        row.outstandingMinor,
        row.currentMinor,
        row.overdue1To15Minor,
        row.overdue16To30Minor,
        row.overdue31To60Minor,
        row.overdue61To90Minor,
        row.overdueOver90Minor,
        row.oldestUnpaidDueDate ?? "",
        row.lastReminderAt ? new Date(row.lastReminderAt).toISOString() : "",
      ]);
    }
    return {
      filename: `receivable-ageing-${args.locale}.csv`,
      contentType: "text/csv;charset=utf-8" as const,
      content: createCsv([header, ...body]),
      rowCount: rows.length,
      truncated,
...
export const adjustmentsCsv = query({
  args: { fromAt: v.number(), toAt: v.number(), locale: localeArg },
  returns: exportPayload,
  handler: async (ctx, args) => {
    await requireOwner(ctx);
    if (args.fromAt >= args.toAt) throw new Error("Invalid adjustment range");
    const source = (
      await Promise.all(
        (["posted", "voided"] as const).map((status) =>
          ctx.db
            ...
            .withIndex("by_status_and_postedAt", (q) =>
              q
                .eq("status", status)
                .gte("postedAt", args.fromAt)
                .lt("postedAt", args.toAt),
            )
            .order("desc")
            .take(REPORT_ROW_LIMIT + 1),
        ),
    ...
    return {
      filename: `finance-adjustments-${args.locale}.csv`,
      contentType: "text/csv;charset=utf-8" as const,
      content: createCsv([header, ...body]),
      rowCount: rows.length,
      truncated,
...
export const cashClosingsCsv = query({
  args: { locale: localeArg },
  returns: exportPayload,
  handler: async (ctx, args) => {
    await requireOwner(ctx);
    ...
    return {
      filename: `cash-closings-${args.locale}.csv`,
      contentType: "text/csv;charset=utf-8" as const,
      content: createCsv([header, ...body]),
      rowCount: rows.length,
      truncated,
...
export const campaignOutcomesCsv = query({
  args: { campaignId: v.id("dueReminderCampaigns"), locale: localeArg },
  returns: exportPayload,
  handler: async (ctx, args) => {
    await requireOwner(ctx);
    ...
    for (const row of rows) {
      ...
      body.push([
        ...
        student?.studentNumber ?? "",
        student?.displayName ?? "",
        row.overdueMinorSnapshot,
        row.locale,
        row.segmentCount,
        row.status,
        row.suppressionReason ?? "",
        row.lastAttemptAt ? new Date(row.lastAttemptAt).toISOString() : "",
      ]);
    }
    return {
      filename: `campaign-${campaign.campaignNumber}-${args.locale}.csv`,
      contentType: "text/csv;charset=utf-8" as const,
      content: createCsv([header, ...body]),
      rowCount: rows.length,
      truncated,
