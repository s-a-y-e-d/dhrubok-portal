...
async function fixture(t: ReturnType<typeof convexTest>) {
  return await t.run(async (ctx) => {
    ...
    const studentId = await ctx.db.insert("students", { studentNumber: "ST-1", displayName: "Student", loginEmail: "student@example.com", normalizedLoginEmail: "student@example.com", schoolCollege: "School", currentClass: "10", guardianName: "Father", guardianPhone: "8801712345678", normalizedGuardianPhone: "8801712345678", guardianRelationship: "father", motherName: "Mother", motherPhone: "8801812345678", smsRecipient: "both", preferredSmsLocale: "en", admissionDate: "2026-01-01", status: "active", searchText: "student", createdAt: now, updatedAt: now, createdByAccountId: ownerAccountId, updatedByAccountId: ownerAccountId });
...
describe("finance invariants", () => {
  ...
  it("handles partial allocation, advance credit, future application, and void reversal", async () => {
    ...
    const paymentMessages = await t.run((ctx) => ctx.db.query("smsMessages").withIndex("by_studentId_and_createdAt", (q) => q.eq("studentId", data.studentId)).collect());
    expect(paymentMessages.map((message) => message.normalizedRecipient).sort()).toEqual(["8801712345678", "8801812345678"]);
