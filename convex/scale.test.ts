...
describe("realistic coaching-centre scale", () => {
  it("serves summaries and bounded queues with 100 students, 50 batches, 15 teachers, and 100 SMS events", async () => {
    ...
    await t.run(async (ctx) => {
      ...
      const courseId = await ctx.db.insert("courses", { code: "SCALE", slug: "scale", nameBn: "Scale", nameEn: "Scale", shortDescriptionBn: "", shortDescriptionEn: "", descriptionBn: "", descriptionEn: "", status: "active", isPublic: false, publicSortOrder: 0, createdAt: now, updatedAt: now, createdByAccountId: ownerId, updatedByAccountId: ownerId });
      const teacherIds = [];
      ...
      for (let index = 0; index < 50; index += 1) {
        const batchId = await ctx.db.insert("batches", { courseId, code: `B-${index}`, slug: `batch-${index}`, nameBn: `Batch ${index}`, nameEn: `Batch ${index}`, startDate: "2026-01-01", status: "active", admissionOpen: false, isPublic: false, publicSortOrder: index, createdAt: now, updatedAt: now });
      ...
      for (let index = 0; index < 100; index += 1) {
        ...
        await ctx.db.insert("enrolments", { studentId, courseId, batchId: batchIds[index % batchIds.length], enrolledOn: "2026-01-01", status: "active", createdAt: now, updatedAt: now, createdByAccountId: ownerId });
