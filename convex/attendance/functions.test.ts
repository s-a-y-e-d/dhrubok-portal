...
async function fixture(t: ReturnType<typeof convexTest>) {
  return await t.run(async (ctx) => {
    ...
    const courseId = await ctx.db.insert("courses", { academicSessionId, code: "HSC", slug: "hsc", nameBn: "এইচএসসি", nameEn: "HSC", shortDescriptionBn: "", shortDescriptionEn: "", descriptionBn: "", descriptionEn: "", status: "active", isPublic: true, publicSortOrder: 1, createdAt: now, updatedAt: now, createdByAccountId: ownerAccountId, updatedByAccountId: ownerAccountId });
    ...
    const batchId = await ctx.db.insert("batches", { academicSessionId, courseId, code: "B1", slug: "b1", nameBn: "ব্যাচ ১", nameEn: "Batch 1", status: "active", admissionOpen: true, isPublic: true, publicSortOrder: 1, createdAt: now, updatedAt: now });
    const studentIds = [];
    for (const [index, status] of ["present", "absent"].entries()) {
      ...
      const enrolmentId = await ctx.db.insert("enrolments", { studentId, courseId, batchId, academicSessionId, enrolledOn: "2026-01-01", status: "active", createdAt: now, updatedAt: now, createdByAccountId: ownerAccountId });
...
describe("immutable attendance submission", () => {
  ...
  it("rejects a schedule from a different batch", async () => {
    ...
    const foreignScheduleId = await t.run(async (ctx) => {
      ...
      const foreignBatchId = await ctx.db.insert("batches", { academicSessionId: batch!.academicSessionId, courseId: batch!.courseId, code: "B2", slug: "b2", nameBn: "ব্যাচ ২", nameEn: "Batch 2", status: "active", admissionOpen: true, isPublic: true, publicSortOrder: 2, createdAt: now, updatedAt: now });
