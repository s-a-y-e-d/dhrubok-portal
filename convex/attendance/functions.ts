...
const sessionSummary = v.object({
  ...
  presentCount: v.union(v.number(), v.null()),
  lateCount: v.union(v.number(), v.null()),
  absentCount: v.union(v.number(), v.null()),
});
...
async function summarizeSession(
  ...
  const teacher = await ctx.db.get("teachers", session.teacherId);
  return {
    ...
    presentCount: session.presentCount ?? null,
    lateCount: session.lateCount ?? null,
    absentCount: session.absentCount ?? null,
  };
}

async function eligibleEnrolments(
  ...
  const rows = await ctx.db
    .query("enrolments")
    .withIndex("by_batchId_and_status", (q) => q.eq("batchId", batchId))
    .take(500);
  return rows.filter(
    (row) =>
      row.enrolledOn <= sessionDate &&
      (!row.endedOn || row.endedOn >= sessionDate),
  );
}
...
export const createSession = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (account.role === "teacher" && account.teacherId !== args.teacherId)
    ...
    if (args.scheduleId) {
      ...
      if (
        schedule.batchId !== args.batchId ||
        schedule.teacherId !== args.teacherId ||
        schedule.subjectId !== args.subjectId
      )
        throw new Error("Schedule does not match the session scope");
      if (
        args.sessionDate < schedule.effectiveFrom ||
        (schedule.effectiveUntil && args.sessionDate > schedule.effectiveUntil)
      )
        throw new Error("Schedule is not effective on this date");
      const weekday = new Date(
        `${args.sessionDate}T12:00:00+06:00`,
      ).getUTCDay();
      const dhakaMinutes = (timestamp: number) => {
        ...
        return (
          Number(parts.find((part) => part.type === "hour")?.value ?? -1) * 60 +
          Number(parts.find((part) => part.type === "minute")?.value ?? -1)
        );
      };
      const dhakaDate = (timestamp: number) =>
      ...
      if (
        ...
        dhakaDate(args.startsAt) !== args.sessionDate ||
        dhakaDate(args.endsAt) !== args.sessionDate ||
        dhakaMinutes(args.startsAt) !== schedule.startMinutes ||
        dhakaMinutes(args.endsAt) !== schedule.endMinutes
      )
        throw new Error("Session date or time does not match the schedule");
    }
    const sessionKey = args.scheduleId
      ? `schedule:${args.scheduleId}:${args.sessionDate}`
      : `batch:${args.batchId}:${args.startsAt}`;
    const duplicate = await ctx.db
      .query("classSessions")
      .withIndex("by_sessionKey", (q) => q.eq("sessionKey", sessionKey))
      .unique();
    if (duplicate) return duplicate._id;
    const roster = await eligibleEnrolments(
      ctx,
      args.batchId,
      args.sessionDate,
    );
...
export const getRoster = query({
  ...
  handler: async (ctx, args) => {
    ...
    const enrolments = await eligibleEnrolments(
      ctx,
      session.batchId,
      session.sessionDate,
    );
    ...
    const statusByStudent = new Map(
      existing.map((row) => [row.studentId, row.status]),
    );
    const students = [];
    ...
    return { session: await summarizeSession(ctx, session), students };
  },
});
...
export const openSession = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const { account } = await requireOwnerOrAssignedTeacher(
      ctx,
      session.batchId,
    );
    if (session.status !== "scheduled" || session.sessionDate !== dhakaDate()) {
      throw new Error(
    ...
    await writeAudit(ctx, {
      ...
      metadata: { rosterCount: enrolments.length },
    });
    return null;
  },
});
...
export const submit = mutation({
  ...
  handler: async (ctx, args) => {
    if (args.records.length > 500)
      throw new Error("Attendance roster is too large");
    const session = await ctx.db.get("classSessions", args.sessionId);
    if (!session) throw new Error("Session not found");
    const { account } = await requireOwnerOrAssignedTeacher(
      ctx,
    ...
    const enrolmentByStudent = new Map(
      enrolments.map((row) => [row.studentId, row]),
    );
    if (args.records.length !== enrolments.length)
      throw new Error("Every eligible student must be marked exactly once");
    const seen = new Set<string>();
    for (const record of args.records) {
      if (seen.has(record.studentId))
        throw new Error("Duplicate student in attendance");
      seen.add(record.studentId);
      if (!enrolmentByStudent.has(record.studentId))
    ...
    for (const record of args.records) {
      ...
      await ctx.db.insert("attendanceRecords", {
        ...
        submittedAt: now,
        submittedByAccountId: account._id,
      });
    ...
    await writeAudit(ctx, {
      ...
      entityType: "classSession",
      entityId: session._id,
      summary: "Attendance submitted permanently",
      metadata: { presentCount, lateCount, absentCount },
    });
  ...
  handler: async (ctx, args) => {
      ...
      account.role === "teacher"
        ? await ctx.db
            ...
            .withIndex("by_teacherId_and_sessionDate", (q) =>
              q
                .eq("teacherId", account.teacherId)
        ...
        : account.role === "owner"
          ? await ctx.db
              ...
              .withIndex("by_sessionDate", (q) =>
                q.eq("sessionDate", args.sessionDate),
              )
              .take(100)
          : [];
    ...
    return await Promise.all(
      rows
        ...
        .map((session) => summarizeSession(ctx, session)),
    );
  },
});
...
export const myHistory = query({
  ...
  handler: async (ctx, args) => {
    ...
    const result = await ctx.db
      .query("attendanceRecords")
      .withIndex("by_studentId_and_submittedAt", (q) =>
