...
export default async function PublicHome({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <>
    ...
    <section className="section container" aria-labelledby="batches-title"><div className="section-heading"><div><p className="eyebrow">{bn ? "ভর্তি চলছে" : "Admissions open"}</p><h2 id="batches-title">{bn ? "খোলা ব্যাচ" : "Open batches"}</h2></div></div>{batches.length ? <div className="card-grid compact">{batches.map((batch) => <article className="content-card" key={batch.id}><p className="eyebrow">{batch.courseName.value}</p><h3>{batch.name.value}</h3>{batch.room.value && <p>{batch.room.value}</p>}<Link href={`/${locale}/courses/${batch.courseSlug}`}>{bn ? "কোর্স ও ব্যাচ দেখুন" : "View course and batch"}</Link></article>)}</div> : <p className="empty-panel">{bn ? "এই মুহূর্তে কোনো পাবলিক ব্যাচে ভর্তি খোলা নেই।" : "No public batch is currently open for admission."}</p>}</section>
