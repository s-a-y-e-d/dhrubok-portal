...
export function AdmissionForm({ locale }: { locale: "bn" | "en" }) {
  ...
  return <><Script src="https://challenges.cloudflare.com/turnstile/v0/api.js?render=explicit" strategy="afterInteractive" onLoad={renderTurnstile} onReady={renderTurnstile} /><form className="operation-form" onSubmit={onSubmit}>
    ...
    <fieldset><legend>{bn ? "অভিভাবকের তথ্য" : "Guardian information"}</legend><div className="form-grid"><label>{bn ? "অভিভাবকের নাম" : "Guardian name"}<input name="guardianName" required maxLength={120} /></label><label>{bn ? "সম্পর্ক" : "Relationship"}<input name="guardianRelationship" required maxLength={80} /></label><label>{bn ? "মোবাইল নম্বর" : "Mobile number"}<input name="guardianPhone" required inputMode="tel" /></label><label>{bn ? "SMS ভাষা" : "SMS language"}<select name="preferredSmsLocale" defaultValue={locale}><option value="bn">বাংলা</option><option value="en">English</option></select></label></div></fieldset>
