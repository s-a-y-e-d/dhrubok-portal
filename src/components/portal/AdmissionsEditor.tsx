...
export function AdmissionsEditor({ locale }: { locale: "bn" | "en" }) {
  ...
  const applications = useQuery(api.admissions.owner.listApplications, {
    status,
  ...
  const detail = useQuery(
    api.admissions.owner.getApplication,
  ...
  async function execute(work: () => Promise<unknown>, closeAfter = false) {
    ...
    try {
      ...
      setFeedback({
        ok: true,
        text: bn ? "আবেদন আপডেট হয়েছে।" : "Application updated.",
      });
      setDecision(null);
      setReason("");
    ...
    <>
      <header className="portal-page-header">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="flex max-w-3xl flex-col gap-2">
            <h1>
              {bn ? "আবেদন পর্যালোচনা ও ভর্তি" : "Review and admit applicants"}
            </h1>
            <p>
              {bn
                ? "নতুন আবেদন যাচাই করুন, ডুপ্লিকেট সতর্কতা দেখুন এবং এক ধাপে ভর্তি সম্পন্ন করুন।"
                : "Review new applications, check duplicate warnings, and complete admission in one workflow."}
            </p>
          </div>
          <Button
            onClick={() => {
              setSelectedId("");
              setDirectAdmission(true);
            }}
          >
      ...
      <Card className="border-[var(--border)] bg-[var(--canvas)] shadow-none">
        <CardHeader className="gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-col gap-1">
            ...
            <CardDescription>
              {bn
                ? "একটি সারি নির্বাচন করলে পর্যালোচনার ড্রয়ার খুলবে।"
                : "Select a row to open its review drawer."}
            </CardDescription>
          </div>
          <ToggleGroup
            type="single"
            ...
            onValueChange={(value) => {
              if (!value) return;
              setStatus(value as "new" | "under_review");
              setSelectedId("");
            }}
            variant="outline"
            aria-label={bn ? "আবেদনের অবস্থা" : "Application status"}
          >
            <ToggleGroupItem value="new">{bn ? "নতুন" : "New"}</ToggleGroupItem>
            <ToggleGroupItem value="under_review">
              {bn ? "পর্যালোচনায়" : "Under review"}
            </ToggleGroupItem>
          </ToggleGroup>
        </CardHeader>
          ...
          {applications.page.length ? (
            <>
              <div className="hidden md:block">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead>{bn ? "আবেদনকারী" : "Applicant"}</TableHead>
                      <TableHead>{bn ? "অভিভাবক" : "Guardian"}</TableHead>
                      <TableHead>{bn ? "পছন্দ" : "Requested"}</TableHead>
                      <TableHead>{bn ? "জমা হয়েছে" : "Submitted"}</TableHead>
                      ...
                      <TableRow
                        ...
                        onKeyDown={(event) => {
                          if (event.key === "Enter" || event.key === " ") {
                            event.preventDefault();
                            choose(application.applicationId);
                          }
                        }}
                      >
                        <TableCell>
                          <div className="flex min-w-48 flex-col gap-1">
                            <strong>{application.studentDisplayName}</strong>
                        ...
                        <TableCell>
                          <Badge
                            variant={
                              application.status === "new" ? "info" : "warning"
                            }
                          >
              ...
              <div className="grid gap-3 px-4 pb-4 md:hidden">
                {applications.page.map((application) => (
                  ...
                  >
                    ...
                    <span className="w-full border-t border-[var(--border-muted)] pt-3 text-xs text-muted-foreground">
                      {dateFormatter.format(application.submittedAt)}
                    </span>
                  </button>
                ))}
            ...
            <div className="p-6">
              <EmptyState
                title={
                  bn
                    ? "এই অবস্থায় কোনো আবেদন নেই"
                    : "No applications in this state"
                }
                description={
                  bn
                    ? "অন্য অবস্থা নির্বাচন করে দেখুন।"
                    : "Try selecting the other status."
                }
              />
            </div>
          )}
          ...
          {selectedId && detail === undefined ? (
            <div className="p-6">
              <SheetTitle className="sr-only">
                {bn ? "আবেদন লোড হচ্ছে" : "Loading application"}
              </SheetTitle>
            ...
            <>
              <SheetHeader className="border-b border-[var(--border)] p-6 pe-14">
                <div className="flex flex-wrap items-center gap-2">
                  <Badge variant={detail.status === "new" ? "info" : "warning"}>
                    {detail.status === "new"
                      ? bn
                        ? "নতুন"
                        : "New"
                ...
                <SheetDescription>
                  {bn
                    ? "আবেদনের তথ্য যাচাই করে পরবর্তী পদক্ষেপ নিন।"
                    : "Verify the application details and choose the next action."}
                </SheetDescription>
              </SheetHeader>
              ...
              <div className="flex flex-col gap-6 px-6 pb-6">
                <section className="flex flex-col gap-3">
                  <h3 className="text-base font-semibold">
                    {bn ? "আবেদনকারীর তথ্য" : "Applicant details"}
                  </h3>
                  <dl className="grid gap-4 rounded-lg border p-4 sm:grid-cols-2">
                    ...
                    <div>
                      <dt className="text-xs text-muted-foreground">
                        {bn ? "শিক্ষার্থীর ফোন" : "Student phone"}
                      </dt>
                      <dd className="mt-1 text-sm font-medium">
                        {detail.studentPhone || "—"}
                      </dd>
                    ...
                    <div>
                      <dt className="text-xs text-muted-foreground">
                        {bn ? "স্কুল বা কলেজ" : "School or college"}
                      </dt>
                      <dd className="mt-1 text-sm font-medium">
                        {detail.schoolCollege}
                      </dd>
                    </div>
                    <div>
                      <dt className="text-xs text-muted-foreground">
                        {bn ? "বর্তমান শ্রেণি" : "Current class"}
                      </dt>
                      <dd className="mt-1 text-sm font-medium">
                        {detail.currentClass}
                      </dd>
                  ...
                  {detail.applicantNote ? (
                    <p className="rounded-lg border p-4 text-sm">
                      <span className="font-medium">
                        {bn ? "আবেদনকারীর নোট:" : "Applicant note:"}
                      </span>{" "}
                      {detail.applicantNote}
                    </p>
                  ) : null}
                ...
                >
                  <div className="flex flex-col gap-1">
                    <h3 className="text-base font-semibold">
                      {bn
                        ? "অভ্যন্তরীণ ভর্তি তথ্য"
                        : "Internal admission details"}
                    </h3>
                    <p className="text-sm text-muted-foreground">
                      {bn
                        ? "গ্রহণ করলে শিক্ষার্থী, এনরোলমেন্ট, চার্জ ও সংরক্ষিত পোর্টাল অ্যাকাউন্ট তৈরি হবে।"
                        : "Acceptance creates the student, enrolment, charges, and reserved portal account."}
                    </p>
                  </div>
                  <FieldGroup className="grid gap-5 sm:grid-cols-2">
                    <Field>
                      <FieldLabel htmlFor="review-student-number">
                        {bn ? "শিক্ষার্থী আইডি" : "Student ID"}
                      </FieldLabel>
                      <Input
                        id="review-student-number"
                        name="studentNumber"
                        required
                    ...
                    <Field>
                      <FieldLabel htmlFor="review-admission-date">
                    ...
                    <Field>
                      <FieldLabel htmlFor="review-initial-charge">
                        {bn
                          ? "প্রাথমিক ভর্তি ফি (৳)"
                          : "Initial admission fee (BDT)"}
                      </FieldLabel>
                      <Input
                        id="review-initial-charge"
                        name="initialCharge"
                        type="number"
                        min="0"
                        step="0.01"
                    ...
                    <Field>
                      <FieldLabel htmlFor="review-course">
                    ...
                    <Field>
                      <FieldLabel htmlFor="review-batch">
                    ...
                    <Field>
                      <FieldLabel htmlFor="review-first-billing-month">
                        {bn ? "প্রথম বিলিং মাস" : "First billing month"}
                      </FieldLabel>
                      <Input
                        id="review-first-billing-month"
                        type="month"
                    ...
                    <Field>
                      <FieldLabel htmlFor="review-monthly">
                    ...
                    <Field className="sm:col-span-2">
                      ...
                      <Textarea id="review-note" name="internalNote" rows={3} />
                      <FieldDescription>
                        {bn
                          ? "শুধু অভ্যন্তরীণ ব্যবহারের জন্য।"
                  ...
                  <div className="sticky bottom-0 flex flex-col-reverse gap-2 border-t border-[var(--border)] bg-background py-4 sm:flex-row sm:flex-wrap sm:justify-end">
                    ...
                    <Button
                      variant="secondary"
                      type="button"
                      disabled={busy || detail.status === "under_review"}
                      onClick={() =>
                        void execute(() =>
                          review({
                            applicationId: detail.applicationId,
                    ...
                    >
                      {bn ? "পর্যালোচনায় রাখুন" : "Mark under review"}
                    </Button>
                    <Button type="submit" disabled={busy}>
                      {busy ? <Spinner data-icon="inline-start" /> : null}
                      {bn ? "গ্রহণ ও ভর্তি করুন" : "Accept and admit"}
                    </Button>
                  </div>
                </form>
              </div>
            ...
            <div className="p-6">
              <SheetTitle className="sr-only">
                {bn ? "আবেদন পাওয়া যায়নি" : "Application not found"}
              </SheetTitle>
              <EmptyState
                title={bn ? "আবেদন পাওয়া যায়নি" : "Application not found"}
              />
            </div>
          )}
        </SheetContent>
      ...
      <Sheet open={directAdmission} onOpenChange={setDirectAdmission}>
        <SheetContent className="w-[min(760px,calc(100%-24px))] gap-0 p-0">
          <SheetHeader className="border-b border-[var(--border)] p-6 pe-14">
            <SheetTitle>
              {bn ? "নতুন শিক্ষার্থী ভর্তি" : "Admit a new student"}
            </SheetTitle>
            <SheetDescription>
              {bn
                ? "পাবলিক আবেদন ছাড়াই শিক্ষার্থী ও এনরোলমেন্ট তৈরি করুন।"
                : "Create a student and enrolment without a public application."}
            </SheetDescription>
          </SheetHeader>
          <div className="px-6 pb-6 pt-5">
            <DirectAdmissionForm
              locale={locale}
              onCancel={() => setDirectAdmission(false)}
            />
          </div>
        </SheetContent>
      </Sheet>
      ...
      >
        <AlertDialogContent>
          <form
            ...
            onSubmit={(event) => {
              event.preventDefault();
              if (!detail || !decision || !reason.trim()) return;
              void execute(
                () =>
                  decision === "reject"
                    ? reject({
          ...
          >
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                {bn
                  ? "এই সিদ্ধান্তের কারণ লিখুন। আবেদনটি বর্তমান ইনবক্স থেকে সরিয়ে দেওয়া হবে।"
                  : "Provide a reason for this decision. The application will leave the current inbox."}
              </AlertDialogDescription>
            </AlertDialogHeader>
            <Field>
